...
import { SERVICES_QUERY } from "@/lib/tinaQueries";
...
export default function Services() {
  const { data: apiData, isLoading: pageLoading } = usePageContent("services");
  const { data: home, isLoading: homeLoading } = usePageContent("home");

  const tinaData = useMemo(() => ({ services: apiData }), [apiData]);
      ...
      <section className="py-16 bg-white">
        <div className="max-w-[1100px] mx-auto px-4 sm:px-6 lg:px-8">
          <h2
            ...
            data-tina-field={tinaRef ? tinaField(tinaRef, "coreServices") : undefined}
              ...
              {page.insights.items.map((insight: any, idx: number) => {
                ...
                return (
                  <div key={idx} data-testid={`insight-${idx}`} className="bg-white border border-gray-200 rounded-sm p-8 text-center flex flex-col">
                    ...
                    <p
                      data-tina-field={tinaField(insight, "description")}
